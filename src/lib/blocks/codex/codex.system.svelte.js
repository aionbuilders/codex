...
export class DataTransformSystem extends System {
    ...
    handleInput(raw) {
        ...
        console.log(
            "Unknown data format received in DataTransformSystem:",
            raw,
        );
    ...
    handleStructuredData(data) {
        switch (data.type) {
            case "json":
                return this.transformJSON(data.content);
            case "html":
                return this.transformHTML(data.content);
            case "markdown":
                return this.transformMarkdown(data.content);
            case "text":
                return this.transformText(data.content);
            case "file":
                return this.handleFileData(data);
            default:
                return this.handleTextFallback(String(data.content));
        }
    }
    ...
    detectAndTransformText(text) {
        ...
        switch (format) {
            case "json":
                return this.transformJSON(text);
            case "html":
                return this.transformHTML(text);
            case "markdown":
    ...
    transformHTML(html) {
        ...
        return {
            ...
            blocks: blocks.filter(
                (block) => block.content || block.type === "linebreak",
            ),
            metadata: {
