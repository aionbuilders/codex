 ...
 * @property {import('../../states/strategy.svelte').Strategy[]} [strategies] - Initial strategies to be added to the codex.
 * @property {typeof import('../../states/system.svelte').System[]} [systems] - Initial systems to be added to the codex.
...
/**
 * @typedef {import('../block.svelte').Block} Block
 */

import CodexComponent from "./Codex.svelte";
import { MegaBlock } from "../block.svelte";
import { CodexSelection } from "../../states/selection.svelte";
import { codexStrategies } from "./codex.strategies";
...
export class Codex extends MegaBlock {
    ...
    constructor(init = {}) {
        ...
        this.preset =
            init.preset || init.preset === null ? init.preset : PlainPreset;
        ...
        this.omited = {
            ...
            strategies:
                init.omit
                    ?.filter((name) => name.startsWith("strategy:"))
                    .map((name) => name.replace("strategy:", "")) || [],
            systems:
                init.omit
            ...
            $effect(() => {
                if (this.element) {
                    untrack(() => {
                        ...
                        const Systems = [
                            ...(this.preset?.systems.filter(
                                (s) =>
                                    !this.omited.systems.includes(
                                        s.manifest.name,
                                    ) &&
                                    !(this.init.systems || []).find(
                                        (s2) =>
                                            s2.manifest.name ===
                                            s.manifest.name,
                                    ),
                        ...
                        this.systems = Systems.map(
                            (SystemClass) => new SystemClass(this),
                        );

                        const inits = this.systems
                            .filter((s) => s.handlers.has("init"))
                            .map((s) => s.handlers.get("init"));
    ...
    get blocks() {
        return /** @type {Array<new (...args: any[]) => Block>} */ ([
            ...(this.preset?.blocks.filter(
                (b) =>
                    !this.omited.blocks.includes(b.manifest.type) &&
                    !(this.init.blocks || []).find(
                        (b2) => b2.manifest.type === b.manifest.type,
                    ),
            ) || []),
            ...(this.init?.blocks || []),
        ]);
    ...
    get strategies() {
        return [
            ...
            ...(this.preset?.strategies?.filter(
                (s) => !this.omited.strategies.includes(s.name),
            ) || []),
    ...
    /** @type {import('../../states/system.svelte').System[]} */
    systems = $state([]);
    ...
    handleEvent = async (e, eventType, strategyTag) => {
        try {
            ...
            if (currentParent && currentParent instanceof MegaBlock) {
                if (eventType === "onbeforeinput" && currentParent === this)
                    e.preventDefault();
                ...
                // @ts-ignore
                const block = this.selection?.anchoredBlocks.findLast(
                    // @ts-ignore
    ...
    effect = (ops) => {
        ...
        if (!current)
            return console.warn("No current transaction in history for effect");
        if (!ops?.length)
            return console.warn("No operations provided for effect");
    ...
    call(name, ...args) {
        ...
        const method = this.systems
            .find((s) => s.methods.has(name))
            ?.methods.get(name);
        console.log(
            "Calling method:",
            name,
    ...
    focus = (focus, options = {}) =>
        queueMicrotask(() => {
            const {
                start = focus.offset || 0,
                end = focus.offset || focus.start || 0,
            } = focus;

            const blockId =
                typeof focus.block === "string" ? focus.block : focus.block?.id;
                ...
                const tx =
                    options.tx instanceof Transaction
                        ...
                        : this.history.current ||
                          this.history.transactions[this.history.index] ||
                          null;
                if (!tx)
                    throw new Error(
                        "No transaction available for focus operation",
                    );
                tx.selectionAfter = {
                    ...focus,
                    start,
                    end,
                    ...(focus.block ? { block: blockId } : {}),
                };
            }

            const block =
                typeof focus.block === "string"
                    ? this.recursive.find((b) => b.id === focus.block)
                    : focus.block || this;
    ...
    getFocusData(f) {
        ...
        const startBlock = this.children.find(
            (b) => b.start <= start && b.end >= start,
        );
        const endBlock =
            start === end
                ? startBlock
                : this.children.find((b) => b.start <= end && b.end >= end);
        ...
        if (startBlock && endBlock) {
            ...
            return {
                startElement: startFocus.startElement,
                startOffset: startFocus.startOffset,
                endElement: endFocus.endElement,
                endOffset: endFocus.endOffset,
            };
        }
    }
    ...
    values = $derived({
        ...
        json: {
            type: "codex",
            children: this.children.map((c) => c.values.json),
            root: true,
        },
    });
