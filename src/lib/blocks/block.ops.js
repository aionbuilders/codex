import { Operation, Operations } from "../utils/operations.utils";

/** @typedef {import('./block.svelte').MegaBlock} MegaBlock */

/**
 * @typedef {Object} BlocksInsertionData
 * @property {import('./block.svelte').BlockData[]} [blocks]
...
export class BlocksInsertion extends Operation {
    ...
    undo() {
        return this.ops(
            new BlocksRemoval(this.block, {
                ids: this.results.map((b) => b.id),
            }),
        );
    }
}
...
export class BlocksRemoval extends Operation {
    ...
    undo() {
        return this.ops(
            new BlocksInsertion(this.block, {
                blocks: this.data.blocks || [],
                offset: this.data.blocks?.[0]?.i || 0,
            }),
        );
    }
}
 ...
 * @property {import('./block.svelte').BlockData[]} [blocks] - The blocks to replace.
 */
...
export class BlocksReplacement extends Operation {
    ...
    constructor(block, data) {
        ...
        const blocks = block.children.filter(
            (b) => b.i >= data.from && b.i < data.to,
        );
        this.data.oldBlocks = blocks.map((b) => ({
            ...b.data(),
            i: b.i,
            index: b.index,
        }));
    ...
    undo() {
        return this.ops(
            new BlocksReplacement(this.block, {
                from: this.data.from,
                to: this.data.from + (this.data.blocks?.length || 0),
                blocks: this.data.oldBlocks || [],
            }),
        );
    }
}
