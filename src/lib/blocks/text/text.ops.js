 ...
 * @typedef {import('../codex/codex.svelte').Codex} CodexBlock
 */
...
export class TextEdition extends Operation {
    ...
    constructor(block, data) {
        ...
        this.metadata.op = !this.data.text
            ? "delete"
        ...
        if (this.metadata.op === "delete") {
            this.deleted = this.block.text.slice(
                this.data.from,
    ...
    get debug() {
        if (!this.data.text)
            return `Delete ${this.data.from} to ${this.data.to ?? this.data.from}`;
        if (!this.data.to || this.data.to === this.data.from)
            return `Insert "${this.data.text}" at ${this.data.from}`;
    ...
    undo() {
        if (this.metadata.op === "delete") {
            ...
            return this.ops(
                new TextEdition(this.block, {
                    from: this.data.from,
                    to: this.data.from + this.inserted.length,
                }),
            );
        } else {
            return this.ops(
