 ...
 * @typedef {import('./paragraph.svelte').ParagraphContent} ParagraphContent
 *  */
...
const replace = (paragraph, content) => {
    /** @type {Text|Linebreak|null} */
    const start = paragraph.children.find((child) => child.selected) || null;
    /** @type {Text|Linebreak|null} */
    const end = paragraph.children.findLast((child) => child.selected) || null;
    if (!start || !end) return null;

    const between = paragraph.children.slice(
        paragraph.children.indexOf(start) + 1,
        paragraph.children.indexOf(end),
    );

    const data = {
        startOffset:
            start.start +
            (start instanceof Text && start.selection
                ? start.selection.start
                : 0),
        endOffset:
            end.start +
            (end instanceof Text && end.selection ? end.selection.end : 0),
...
export const paragraphDeleteStrategy = new Strategy(
    ...
    (codex, context) => {
        ...
        const data = replace(
            paragraph,
            context.event.key === "Enter" ? [{ type: "linebreak" }] : [],
        );
        if (data && data.tx) {
            if (context.event.key === "Enter")
                data.tx.focus({
                    start: data.startOffset + 1,
                    end: data.startOffset + 1,
                    block: paragraph,
                });
            else
                data.tx.focus({
                    start: data.startOffset,
                    end: data.startOffset,
                    block: paragraph,
                });
        }
    },
    ...
    (codex, context) => {
        ...
        if (event.inputType === "insertText") {
            ...
            if (event.data) {
                ...
                if (data?.tx)
                    data.tx.focus({
                        start: (selection?.start || 0) + event.data.length,
                        end: (selection?.start || 0) + event.data.length,
                        block,
                    });
            }
        }
